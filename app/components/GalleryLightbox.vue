...
const props = defineProps<{ images: { file: string, caption: string }[], srcBase: string }>()
...
const src = (file: string) => `${props.srcBase}/${file}`
...
function preloadNeighbours() {
  for (const delta of [1, -1])
    new Image().src = src(props.images[at(delta)]!.file)
}

// Filenames on disk are opaque, so the download is renamed after the caption.
    ...
    <div v-if="isOpen" class="flex h-full items-center justify-center p-[1vh]" @click.self="dialog?.close()">
      <figure class="flex flex-col items-center gap-2">
        <img
          ...
          :alt="current.caption"
          class="max-h-[min(calc(98vh-6rem),900px)] w-auto max-w-full bg-content object-contain shadow-lg"
        >
        <figcaption class="w-full bg-content px-4 py-2 text-center font-header text-sm">
          ...
          <div class="flex flex-wrap items-center justify-center gap-4">
            ...
            <a :href="src(current.file)" :download="downloadName">
              Download
            </a>
